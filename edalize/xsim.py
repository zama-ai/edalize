...
class Xsim(Edatool):
    ...
    argtypes = ["cmdlinearg","plusarg", "vlogdefine", "vlogparam", "generic"]
...
xsim.dir/$(TARGET)/xsimk:
	...
	xelab -timescale 1ns/1ps work.glbl $(TOPLEVEL) -initfile=${XILINX_VIVADO}/data/xsim/ip/xsim_ip.ini -L work -L xpm -prj $(TARGET).prj -snapshot $(TARGET) $(VLOG_DEFINES) $(VLOG_INCLUDES) $(GEN_PARAMS) $(XELAB_OPTIONS) -L unisims_ver

run: xsim.dir/$(TARGET)/xsimk
	xsim $(XSIM_OPTIONS) $(TARGET) $(EXTRA_OPTIONS) -R

run-wave: xsim.dir/$(TARGET)/xsimk
	xsim $(XSIM_OPTIONS) $(TARGET) $(EXTRA_OPTIONS) -t $(XSIM_WAVE_TCL) -wdb $(XSIM_WDB) -onfinish quit

run-gui: xsim.dir/$(TARGET)/xsimk
	xsim $(XSIM_OPTIONS) $(TARGET) $(EXTRA_OPTIONS) -g -t $(XSIM_WAVE_TCL) -wdb $(XSIM_WDB)
"""
...
XSIM_WAVE_TCL = {xsim_wave_tcl}
XSIM_WDB      = {xsim_wdb}
"""

    XSIM_WAVE_TCL_TEMPLATE = """#Auto generated by Edalize
...
"""
    ...
    def _write_config_files(self):
        ...
        with open(os.path.join(self.work_root, "config.mk"), "w") as f:
            ...
            xsim_options_l = self.tool_options.get("xsim_options", [])

            xsim_wdb = os.path.join(self.work_root, "wave.wdb")
            xsim_wave_tcl = os.path.join(self.work_root, "dump_wave.tcl")
            for opt in xsim_options_l:
                if (opt.startswith('-wavetcl')):
                    xsim_wave_tcl = opt

            try:
                idx = xsim_options_l.index(xsim_wave_tcl)
                xsim_options_l.pop(idx)
                xsim_wave_tcl = xsim_wave_tcl.split()[1]
            except ValueError:
                None
 
            xelab_options = " ".join(self.tool_options.get("xelab_options", []))
            xsim_options = " ".join(xsim_options_l)
        ...
        with open(os.path.join(self.work_root, "dump_wave.tcl"), "w") as f:
            f.write(self.XSIM_WAVE_TCL_TEMPLATE)

    def run_main(self):
        run = "run"
        for key, value in self.cmdlinearg.items():
            if (key == 'mode' and value == 'gui'):
                run = "run-gui"
            elif (key == 'mode' and value == 'wave'):
                run = "run-wave"
